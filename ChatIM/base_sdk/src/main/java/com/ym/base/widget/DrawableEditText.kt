...
class DrawableEditText(context: Context, attributeSet: AttributeSet?) :
    ...
    init {
        ...
        if (drawableEnd == null && drawableRight == null) {
            ...
            rightWidth = SizeUtils.dp2px(16f)
            rightHeight = SizeUtils.dp2px(16f)
        } else {
            rightD = drawableEnd ?: drawableRight
            if (rightWidth == 0) rightWidth = SizeUtils.dp2px(16f)
            if (rightHeight == 0) rightHeight = SizeUtils.dp2px(16f)
        }
        ...
        addTextChangedListener {
            setCompoundDrawablesWithIntrinsicBounds(
                leftD, topD,
                if (showByFocus) {
                    if (text.isNullOrBlank() || !hasFocus()) null else rightD
                } else if (it.isNullOrBlank()) null else rightD, bottomD
            )
        }
    }

    private var showByFocus = false
    fun setFocusShow() {
        showByFocus = true
        setCompoundDrawablesWithIntrinsicBounds(leftD, topD, if (text.isNullOrBlank() || !hasFocus()) null else rightD, bottomD)
        setOnFocusChangeListener { _, hasFocus ->
            listener?.onFocusChange(hasFocus)
            if (hasFocus) {
                setCompoundDrawables(leftD, topD, if (text.isNullOrBlank()) null else rightD, bottomD)
            } else {
                setCompoundDrawables(leftD, topD, null, bottomD)
            }
        }
    }

    var listener:FocusListener? = null
    public fun setOnFocusListener(lis:FocusListener){
        listener = lis
    }
