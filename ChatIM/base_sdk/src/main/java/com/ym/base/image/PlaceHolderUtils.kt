...
object PlaceHolderUtils {
   ...
   */
  fun getLoadingHolder(ratio: Float = 1f, width: Int = ScreenUtils.getScreenWidth(), @ColorInt bgColor: Int = Color.parseColor("#F4F4F4")): LayerDrawable {
    ...
    d1.color = bgColor
    //占位图
    val d2 = ResourceUtils.getDrawable(R.drawable.holder_loading_s)
   ...
   * @param bgColor 占位图背景色
   */
  fun getErrorHolder(ratio: Float = 1f, width: Int = ScreenUtils.getScreenWidth(), @ColorInt bgColor: Int = Color.parseColor("#F4F4F4")): LayerDrawable {
    ...
    d1.color = bgColor
    //占位图
    val d2 = ResourceUtils.getDrawable(R.drawable.holder_error_s)
    d2.setBounds(0, 0, size, size)
    //生成LayerDrawable
    return LayerDrawable(arrayOf(d1, d2)).also {
      it.setLayerSize(0, width, height)
