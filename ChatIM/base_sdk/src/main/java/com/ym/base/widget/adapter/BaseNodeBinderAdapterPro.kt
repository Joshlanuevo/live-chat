 ...
 * 数据类型为Any
 */
open class BaseNodeBinderAdapterPro(nodeList: MutableList<Any>? = null) : BaseBinderAdapterPro(nodeList) {
     ...
     */
    inline fun <reified T : Any> addFullItemBinder(baseItemBinder: BaseItemBinderPro<T, *>, callback: DiffUtil.ItemCallback<T>? = null): BaseNodeBinderAdapterPro {
        addItemBinder(T::class.java, baseItemBinder, callback)
        addFullSpanNodeProvider(T::class.java)
        return this
    }
     ...
     * kotlin 可以使用如下方法添加 ItemBinder，更加简单
     */
    inline fun <reified T : Any> addFullItemBinderMany(baseItemBinder: BaseItemBinderPro<T, *>, callback: DiffUtil.ItemCallback<T>? = null): BaseNodeBinderAdapterPro {
        addItemBinderMany(baseItemBinder, callback)
        addFullSpanNodeProvider(baseItemBinder::class.java)
        return this
    }
    ...
    override fun setNewInstance(list: MutableList<Any>?) {
     ...
     */
    override fun setList(list: Collection<Any>?) {
     ...
     * @param data BaseNode
     */
    override fun addData(position: Int, data: Any) {
    ...
    override fun addData(position: Int, newData: Collection<Any>) {
        val nodes = flatData(newData)
        super.addData(position, nodes)
    }

    override fun addData(newData: Collection<Any>) {
        val nodes = flatData(newData)
        super.addData(nodes)
    }
    ...
    override fun setDiffNewData(list: MutableList<Any>?) {
        if (hasEmptyView()) {
            setNewInstance(list)
            return
        }
    ...
    override fun setDiffNewData(diffResult: DiffUtil.DiffResult, list: MutableList<Any>) {
        if (hasEmptyView()) {
            setNewInstance(list)
            return
        }
    ...
    private fun removeChildAt(position: Int): Int {
        ...
        val node = this.data[position]
        // 移除子项
        if (node is BaseNode && !node.childNode.isNullOrEmpty()) {
     ...
     */
    fun nodeAddData(parentNode: Any, data: BaseNode) {
        (parentNode as? BaseNode)?.childNode?.let {
            it.add(data)
     ...
     */
    fun nodeAddData(parentNode: Any, childIndex: Int, data: BaseNode) {
        (parentNode as? BaseNode)?.childNode?.let {
     ...
     */
    fun nodeAddData(parentNode: Any, childIndex: Int, newData: Collection<BaseNode>) {
        (parentNode as? BaseNode)?.childNode?.let {
     ...
     * @param childIndex Int 此位置是相对于其childNodes数据的位置！并不是整个data
     */
    fun nodeRemoveData(parentNode: Any, childIndex: Int) {
        (parentNode as? BaseNode)?.childNode?.let {
     ...
     */
    fun nodeRemoveData(parentNode: Any, childNode: BaseNode) {
        (parentNode as? BaseNode)?.childNode?.let {
            if (parentNode is BaseExpandNode && !parentNode.isExpanded) {
                it.remove(childNode)
                return
     ...
     */
    fun nodeSetData(parentNode: Any, childIndex: Int, data: BaseNode) {
        (parentNode as? BaseNode)?.childNode?.let {
            if (childIndex >= it.size) {
                return
            }
     ...
     */
    fun nodeReplaceChildData(parentNode: Any, newData: Collection<BaseNode>) {
        (parentNode as? BaseNode)?.childNode?.let {
            if (parentNode is BaseExpandNode && !parentNode.isExpanded) {
                it.clear()
                it.addAll(newData)
     ...
     */
    private fun flatData(list: Collection<Any>, isExpanded: Boolean? = null): MutableList<Any> {
    ...
    fun expandAndCollapseOther(
        ...
        var lastPosition: Int = if (parentPosition == -1) {
            ...
            val dataSize = (data[parentPosition] as? BaseNode)?.childNode?.size ?: 0
    ...
    fun findParentNode(node: BaseNode): Int {
        ...
        for (i in pos - 1 downTo 0) {
            ...
            if (tempNode is BaseNode && tempNode.childNode?.contains(node) == true) {
                return i
            }
        }
        return -1
     ...
     */
    @PublishedApi
    internal fun addFullSpanNodeProvider(clazz: Class<*>) {
        fullSpanNodeTypeSet.add(findViewType(clazz))
    }
}