 ...
 * Time:10:07
 */
@Parser(name = "BaseJsonOther")
open class BaseJsonOtherParser<T> : AbstractParser<T> {
    /**
...
//                }
        ...
        try {
            GsonUtils.fromJson(result,BaseBean::class.java)
        } catch (e : Exception) {
            //Json解析过程中发生的错误都这么处理
            val errorString = StringBuilder("BaseJsonOtherParser解析 Url: ").append("\n")
        ...
        return try {
            ...
            val errorString = StringBuilder("BaseJsonOtherParser解析 Url: ").append("\n")
                .append(response.request.url.toString()).append("\n")
                .append("转化为BaseJsonOther失败").append("\n")
                .append(e.message).toString()
            throw JsonSyntaxException(errorString,e).apply { this.printStackTrace() }
        }
    }
