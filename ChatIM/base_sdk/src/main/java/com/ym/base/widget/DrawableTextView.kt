...
import com.ym.base.widget.span.LinkTouchMovementMethod
...
class DrawableTextView(context: Context, attributeSet: AttributeSet?) :
        AppCompatTextView(context, attributeSet), View.OnClickListener {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        event?.let { e ->
            ...
            if ((e.action and MotionEvent.ACTION_MOVE) == MotionEvent.ACTION_MOVE && !rect.contains(e.x.toInt(), e.y.toInt())) return true
            val txt = text
            if (movementMethod != null && txt is Spannable) {
                movementMethod.onTouchEvent(this, txt, e)
            }
        }
