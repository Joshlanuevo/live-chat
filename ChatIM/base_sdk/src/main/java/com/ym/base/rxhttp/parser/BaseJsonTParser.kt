 ...
 * Time:10:07
 */
@Parser(name = "BaseJsonT")
open class BaseJsonTParser<T> : AbstractParser<T> {
    /**
...
//                }
        ...
        //获取泛型类型
        val newType : Type = ParameterizedTypeImpl[BaseBean::class.java,mType]

        //转换类型(转换为BaseResponse判断是否成功)
        return try {
            GsonUtils.fromJson<BaseBean<T>>(result,newType)
        } catch (e : Exception) {
            //Json解析过程中发生的错误都这么处理
            val errorString = StringBuilder("BaseJsonTParser解析Url: ").append("\n")
                .append(response.request.url.toString()).append("\n")
                .append("转化为BaseResponse<T>失败").append("\n")
            ...
            when {
                code == null -> {
                    //Json解析过程中发生的错误都这么处理
                    val errorString = StringBuilder("BaseJsonTParser解析解析 Url: ").append("\n")
                        .append(response.request.url.toString()).append("\n")
                        .append("转化为BaseResponse<T>成功").append("\n")
                ...
                code != ConstBeanCode.SUCESS -> {
                    ...
                    throw ParseException(code.toString(),message,response).also { it.printStackTrace() }
                }
            }
        }.run {
            //Json解析过程中发生的错误都这么处理
            ...
            if (data != null) data else {
                val errorString = StringBuilder("BaseJsonTParser解析解析 Url: ").append("\n")
                    .append(response.request.url.toString()).append("\n")
                    .append("发现解析到 data 字段为null,请确认当前请求是否需要使用当前解析器").append("\n")
                    .toString()
                throw ParseException(code.toString(),errorString,response).also { it.printStackTrace() }
            }
        }
    }
}