...
/**
 *
 * https://github.com/jimmysuncpt/AppBadge/blob/master/app/src/main/java/com/jimmysun/appbadge/BadgeUtils.java
 * 应用角标工具类
 *
 * @author SunQiang
 * @since 2018/11/28
...
public class BadgeUtils {
    ...
    public static boolean setCount(final int count, final Context context, Notification xiaoMiNotification) {
        if (count >= 0 && context != null) {
            Log.d("BRAND", Build.BRAND);
            switch (Build.BRAND.toLowerCase()) {
                case "xiaomi":
                    setXiaomiBadge(count, xiaoMiNotification);
                    return true;
                case "huawei":
                case "honor":
