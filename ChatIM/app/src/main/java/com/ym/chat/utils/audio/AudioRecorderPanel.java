...
public class AudioRecorderPanel implements View.OnTouchListener {
    //最大录音时长
    private int maxDuration = 30 * 1000;
    ...
    private Long downSystemTime = 0l;
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.d("AudioRecorderPanel", "点击=" + newTime + "-------" + downSystemTime + "------" + (newTime - downSystemTime));
                if (newTime - downSystemTime > 1000) {
                    button.setBackgroundResource(R.drawable.shape_session_btn_voice_pressed);
                    startRecord();
                    downSystemTime = newTime;
                } else {
                    Log.e("AudioRecorderPanel", "点击过于频繁");
                }
                break;
    ...
    private void cancelRecord() {
        ...
        if (recordListener != null) {
            recordListener.onRecordFail(ActivityUtils.getTopActivity().getString(R.string.已取消));
        }
        hideRecording();
